...
<body style="background:#E1F0F7">
	...
	<table width="795" border="0">
		<tr class=tr1>
			<td colspan="6" width="795">　　<?php if($id){echo "修改新闻";}else{echo "添加新闻";}?></td>
		</tr>
		<tr class=tr3>
			<td width="130">标题</td><td width="695" align="left"><?php show_fckeditor('news[title]','Title',false,"50",$news->title,300);?></td>
		</tr>
		<tr class=tr3>
			<td width="130">短标题</td><td width="695" align="left"><input value="<?php echo $news->short_title; ?>" type="text" class="required" name="news[short_title]" id="news_short_title"><span id="max_len"></span></td>
		</tr>
		<tr class=tr3>
			<td>分　类</td>
			<td align="left">
		...
		<tr class=tr3>
			<td>过期时间</td>
			<td align="left">
				<input type="text" size="20" name="news[guoqi]" class="date_jquery" value="<?php if($news->guoqi!='')echo date_format($news->guoqi,'Y-m-d');?>">(如果有过期时间再填写)
			</td>
		</tr>
		<tr class=tr3>
			<td>关键词</td>
		...
		<tr class=tr3>
			<td>链接</td>
			<td align="left">
				<input type="text" size="20"  value="<?php echo $news->target_url;?>"  name=news[target_url]>(填写则跳转到链接页面)</td>
		</tr>
		<tr class=tr3>
			<td>缩略图</td>
			<td align="left">
				<input type="file" name="image" id="image"><?php if($news->slt_src!=''){?><a target="_blank" href="<?php echo $news->slt_src;?>">显示缩略图</a><?php } ?> 若前台想显示缩略图请上传(新闻所有图片不得大于1M)</td>
		</tr>	
		<tr class="normal_news tr3">
			<td height=100>简短描述</td><td><?php show_fckeditor('news[description]','Admin',true,"100",$news->description);?></td>
		</tr>
		<tr class="normal_news tr3">
			<td height=265>新闻内容</td><td><?php show_fckeditor('news[news_content]','Admin',true,"265",$news->news_content);?></td>
		</tr>
		<tr class=tr3>

<?php
	...
	if($title!=''){
		$sql = "select * from hoau_contraband where 1=1 and (name like '%".$title."%' or description like '%".$title."%') order by created_at desc";
	}else{
		$sql = "select * from hoau_contraband where 1=1 order by created_at desc";
	}
...
<body style="background:#E1F0F7">
	<table width="795" border="0">
		<tr class="tr1">
			<td colspan="5" width="795">　违禁品管理　<a href="contraband_add.php" style="color:#0000FF">添加违禁品</a>　
		...
		<tr class="tr3" id="<?php echo $records[$i]->id;?>">
			<td><a href="contraband_edit.php?id=<?php echo $records[$i]->id;?>" style="color:#000000; text-decoration:none"><?php echo $records[$i]->name;?></a></td>
			...
			<td>
				...
				<?php }?>
				<a href="contraband_edit.php?id=<?php echo $records[$i]->id;?>" style="color:#000000; text-decoration:none">修改</a> 
				<span style="color:#ff0000; cursor:pointer" class="del" name="<?php echo $records[$i]->id;?>">删除</span>
			</td>
		</tr>
		<? }?>
	</table>
	<input type="hidden" id="db_table" value="hoau_contraband">
	<div class="div_box">
		<table width="795" border="0">

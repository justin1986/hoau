<?php
	...
	$db = get_db();
	$key = $_REQUEST['key'];
	$sql = "select * from hoau_problem where 1=1";
	if(!empty($key)){
...
<body style="background:#E1F0F7">
	<table width="795" border="0">
		<tr class="tr1">
			<td colspan="5" width="795">　　　<a href="problem_edit.php" style="color:#0000FF">发布项目</a>
		...
		<tr class="tr2">
			<td width="300">项目名称</td><td width="180">创建时间</td><td width="50">优先级</td><td width="250">操作</td>
		</tr>
		<? for($i=0;$i<$count;$i++){?>
		<tr class="tr3" id="<?php echo $record[$i]->id;?>">
			<td><a href="question_list.php?id=<?php echo $record[$i]->id;?>"><?php echo $record[$i]->name;?></a></td>
			<td><?php echo date_format($record[$i]->create_time,'Y-m-d');?></td>
			...
			<td><?php if($record[$i]->is_adopt=="1"){?><span style="color:#FF0000;cursor:pointer" class="revocation" name="<?php echo $record[$i]->id;?>">撤消</span><? }?>
				...
				<a href="problem_edit.php?id=<?php echo $record[$i]->id;?>">编辑项目名称</a>
				<a href="question_edit.php?p_id=<?php echo $record[$i]->id;?>" style="color:#000000; text-decoration:none">添加题目</a>
				<a href="question_list.php?id=<?php echo $record[$i]->id;?>" style="color:#000000; text-decoration:none">查看题目</a>
			</td>
		</tr>
		<?  }?>
	</table>
	<input type="hidden" id="db_table" value="hoau_problem">
	<div class="div_box">
		<table width="795" border="0">
